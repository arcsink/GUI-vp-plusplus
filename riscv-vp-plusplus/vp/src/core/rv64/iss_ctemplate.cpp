	...
	do {
		try {
			...
			OP_SWITCH_BEGIN() {
				...
				OP_CASE(AMOSWAP_D) {
					execute_amo_d(instr, PmaAmoClass::AMOSwap, TlmAmoOp::Swap, [](int64_t a, int64_t b) {
						(void)a;
						return b;
					});
				}
				...
				OP_CASE(AMOADD_D) {
					execute_amo_d(instr, PmaAmoClass::AMOArithmetic, TlmAmoOp::Add, [](int64_t a, int64_t b) { return a + b; });
				}
				OP_END();

				OP_CASE(AMOXOR_D) {
					execute_amo_d(instr, PmaAmoClass::AMOLogical, TlmAmoOp::Xor, [](int64_t a, int64_t b) { return a ^ b; });
				}
				OP_END();

				OP_CASE(AMOAND_D) {
					execute_amo_d(instr, PmaAmoClass::AMOLogical, TlmAmoOp::And, [](int64_t a, int64_t b) { return a & b; });
				}
				OP_END();

				OP_CASE(AMOOR_D) {
					execute_amo_d(instr, PmaAmoClass::AMOLogical, TlmAmoOp::Or, [](int64_t a, int64_t b) { return a | b; });
				}
				OP_END();

				OP_CASE(AMOMIN_D) {
					execute_amo_d(instr, PmaAmoClass::AMOArithmetic, TlmAmoOp::MinSigned, [](int64_t a, int64_t b) { return std::min(a, b); });
				}
				OP_END();

				OP_CASE(AMOMINU_D) {
					execute_amo_d(instr, PmaAmoClass::AMOArithmetic, TlmAmoOp::MinUnsigned,
					              [](int64_t a, int64_t b) { return std::min((uint64_t)a, (uint64_t)b); });
				}
				OP_END();

				OP_CASE(AMOMAX_D) {
					execute_amo_d(instr, PmaAmoClass::AMOArithmetic, TlmAmoOp::MaxSigned, [](int64_t a, int64_t b) { return std::max(a, b); });
				}
				OP_END();

				OP_CASE(AMOMAXU_D) {
					execute_amo_d(instr, PmaAmoClass::AMOArithmetic, TlmAmoOp::MaxUnsigned,
					              [](int64_t a, int64_t b) { return std::max((uint64_t)a, (uint64_t)b); });
				}
				OP_END();
