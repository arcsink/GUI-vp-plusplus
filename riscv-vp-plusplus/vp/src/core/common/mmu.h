...
constexpr uint64_t PTE_PBMT = uint64_t(0b11) << 61;
...
struct MMU_T {
	...
	uint8_t last_translation_pbmt = 0;
	...
	void flush_tlb() {
		...
		last_translation_pbmt = 0;
	}

	uint8_t get_last_translation_pbmt() const {
		return last_translation_pbmt;
	}
	...
	uint64_t walk(uint64_t vaddr, MemoryAccessType type, PrivilegeLevel mode) {
		...
		for (int i = vm.levels - 1; i >= 0; --i) {
			...
			pte_t pte;
			if (vm.ptesize == 4)
				pte.value = mem->mmu_load_pte32(pte_paddr, type, vaddr);
			else
				pte.value = mem->mmu_load_pte64(pte_paddr, type, vaddr);

			/* Svpbmt 的 PBMT 位位于 PTE[62:61]，不属于物理页号。
			 * 构造物理地址前必须先清掉，否则 NC/IO 会被误当成物理地址高位。 */
			uint64_t ppn = (pte.value & ~PTE_PBMT) >> PTE_PPN_SHIFT;
			...
			if ((pte & ad) != ad) {
				if (page_fault_on_AD) {
					...
					// too)
					mem->mmu_store_pte32(pte_paddr, pte | ad, type, vaddr);
				}
			}
			...
			last_translation_pbmt = pte.PBMT();
			return paddr;
		}
