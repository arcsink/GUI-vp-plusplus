...
| `!local_hit && !cache_support_dirty`                    | `ReadClean`          | 只接 clean data              | 只能分配 `UC/SC` 等 clean 可读状态  |
| `!local_hit && cache_support_dirty && !cache_support_SD` | `ReadNotSharedDirty` | 不接 `SD`，可接 `UC/UD/SC`   | 分配可读状态，dirty owner 必须回数  |
| `!local_hit && cache_support_dirty && cache_support_SD`  | `ReadShared`         | 可接 `UC/UD/SC/SD` 读响应    | 由 response 决定 `UC/UD/SC/SD`      |
...
CHI Issue H 中还定义了 `WriteUniquePtl/WriteUniqueFull` 这类 Immediate Write：
Requester 当前为 Invalid 时，可以直接把 partial/full line 写数据交给 Home/下级缓存，
它们是“带写数据的写事务”，不是获取本地 unique line 后再写的事务。当前
write-back L1 cache 规则保留 allocate-and-write 模型：

- 需要旧 bytes 的 miss store 用 `ReadUnique`，先取旧 line 再合并写。
- 覆盖整条 line 的 miss store 用 `MakeUnique`，只拿 unique ownership，然后本地写入。
- 如果以后实现 CHI RN-F direct write 或 write-through/no-allocate 优化，才把
  Invalid requester 的 store 映射到 `WriteUniquePtl/WriteUniqueFull`。
...
- Dirty owner 被其他 master snoop 时，必须通过 snoop response 或 writeback
  保证最新数据不会丢。
- CHI 允许 `WriteBackPtl` 表达 `UniqueDirtyPartial`，但当前 `cache_ace` 没有
  UDP/byte-valid line 状态，仍按整条 cache line 写回。
...
| `RNSD`               | `ReadNotSharedDirty`                    | 获取 read data，但不接 `SD`                         |
...
| `DirectWritePath`    | `WriteUniquePtl/WriteUniqueFull`        | Requester 为 Invalid 时直接带写数据到 Home/下级缓存 |
...
- `doc/GUI-VP/testcases/PMA_PBMT_ACE_TEST.md`：记录 Linux 侧 PMA/PBMT/ACE trace 验证方法。
