...
| 变量        | 含义                    | 当前 VP 来源                                      |
| ----------- | ----------------------- | ------------------------------------------------- |
| `memtype`   | `Normal` 或 `Device`    | `pma_attributes::memory_type`                     |
| `cacheable` | 是否允许缓存            | PMA `cacheable`，并受 PBMT NC/IO 覆盖             |
...
当前 VP 中 `tlm_ext_pbmt::ace_cacheable()` 是地址属性合并入口：

```cpp
bool is_coherent_addr =
    pbmt is PMA &&
    pma_memory_type == MainMemory &&
    pma_cacheable &&
    pma_coherent;
```

其中 PBMT `NC/IO/RESERVED` 优先级高于 PMA cacheable/coherent。只要 PBMT 强制
uncached，最终就不能进入 coherent L1 分配。
